...
Example:
```
Monday post at 14:00 → scraped Wednesday 14:00 → 1200 likes
Friday post at 14:00 → scraped Sunday 14:00 → 800 likes
...
│   │   └── entities/social-account.entity.ts
│   │
│   ├── posts/
│   │   ├── posts.module.ts
│   │   ├── posts.controller.ts
...
│       │   ├── generate-content.tool.ts
│       │   └── suggest-schedule.tool.ts
│       └── dto/
│           └── analysis-request.dto.ts
│
├── config/
│   └── ai.config.ts (API keys, model config)
│
└── database/
    └── seeds/
        └── posts.seed.ts (test data)
```
...
```typescript
{
  suggestedContent: string, // "🎯 3 tips for..."
